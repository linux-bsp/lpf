...
### 允许的文档操作
- ✅ 修改已存在的文档（如更新 README.md、CLAUDE.md）
- ✅ 用户明确要求创建的文档
...
# 1. 加载预定义配置
python3 build.py config <config_name>    # 例如：ccm_development_defconfig, tests_full_defconfig
...
#### 产品配置
- `ccm_development` - CCM 开发环境（包含所有调试功能）
- `ccm_production` - CCM 生产环境（优化配置，禁用测试）

#### 测试配置
- `tests_full_defconfig` - 全栈测试（所有模块、所有功能）
- `tests_osal_defconfig` - OSAL 单元测试
- `tests_hal_defconfig` - HAL 单元测试
- `tests_pdl_defconfig` - PDL 单元测试
- `tests_prl_defconfig` - PRL 协议层测试
- `tests_aconfig_defconfig` - ACONFIG 单元测试
- `tests_pconfig_defconfig` - PCONFIG 单元测试
- `tests_system_defconfig` - 系统集成测试
- `tests_stress_defconfig` - 压力测试
- `tests_minimal_defconfig` - 最小化配置（仅核心功能）
