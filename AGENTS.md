...
`core/` contains reusable middleware layers: `osal`, `hal`, `pconfig`, `prl`, `pdl`, `aconfig`, and `test_framework`. `products/ctest/` holds the core test product and most runnable test cases. Configuration presets live under `configs/ctest/`, build support scripts are under `scripts/`, and generated artifacts are written to `_build/`.
...
## Testing Guidelines
Tests are C sources registered through `core/test_framework/` and grouped by scope under `products/ctest/` (`unit`, `system`, `stress`, `performance`). Name test files `test_*.c` and keep module-specific configs alongside them in `Config.in`. Prefer running the smallest defconfig that exercises your change, then verify with `./_build/bin/es-middleware-test --all`.
