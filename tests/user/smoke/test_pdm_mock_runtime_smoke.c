...
#define PDM_MOCK_EXPECTED_DEVICE_COUNT 3U

static int fail(int code, const char *message)
{
	fprintf(stderr, "pdm_mock_runtime_smoke: %s\n", message);
	return code;
}
			 ...
			 uint64_t required_capabilities)
{
	...
	if (info.state != PDM_CTL_DEVICE_STATE_BOUND)
...
static int expect_missing_devices(void)
{
	...
	errno = 0;
	if (pdi_mcu_open(&mcu, "/dev/pdm/mcu1") == 0) {
	...
	errno = 0;
	if (pdi_led_open(&led, "/dev/pdm/led2") == 0) {
...
static int expect_mcu_io(void)
{
	...
	if (info.abi_version != PDM_MCU_ABI_VERSION || info.max_devices == 0) {
		(void)pdi_mcu_close(&mcu);
...
static int expect_led_io(const char *name)
{
	...
	if (info.abi_version != PDM_LED_ABI_VERSION || info.max_devices == 0) {
		(void)pdi_led_close(&led);
...
int main(void)
{
	...
	struct pdm_ctl_info ctl_info;
	struct pdm_ctl_device_info devices[PDM_MOCK_EXPECTED_DEVICE_COUNT];
	uint32_t count = PDM_MOCK_EXPECTED_DEVICE_COUNT;
	int ret;

	if (pdi_ctl_open(&ctl, NULL) != 0)
		return fail(1, "failed to open /dev/pdm_ctl");
	...
	if (ctl_info.abi_version != PDM_CTL_ABI_VERSION ||
	    ctl_info.device_count != PDM_MOCK_EXPECTED_DEVICE_COUNT) {
		(void)pdi_ctl_close(&ctl);
	...
	if (pdi_list_devices(&ctl, devices, &count) != 0 ||
	    count != PDM_MOCK_EXPECTED_DEVICE_COUNT) {
		(void)pdi_ctl_close(&ctl);
		return fail(4, "device list mismatch");
	}

	ret = expect_device(&ctl, "mcu0", PDM_CTL_DEVICE_TYPE_MCU, 0,
			    PDM_CTL_DEVICE_CAP_USER_IOCTL |
				    PDM_CTL_DEVICE_CAP_DEBUGFS |
				    PDM_CTL_DEVICE_CAP_TRANSPORT_CAN);
	if (ret) {
		(void)pdi_ctl_close(&ctl);
		return ret;
	}

	ret = expect_device(&ctl, "status", PDM_CTL_DEVICE_TYPE_LED, 0,
			    PDM_CTL_DEVICE_CAP_USER_IOCTL |
				    PDM_CTL_DEVICE_CAP_DEBUGFS |
				    PDM_CTL_DEVICE_CAP_CONTROL_GPIO);
	if (ret) {
		(void)pdi_ctl_close(&ctl);
		return ret;
	}

	ret = expect_device(&ctl, "activity", PDM_CTL_DEVICE_TYPE_LED, 1,
			    PDM_CTL_DEVICE_CAP_USER_IOCTL |
				    PDM_CTL_DEVICE_CAP_DEBUGFS |
				    PDM_CTL_DEVICE_CAP_CONTROL_PWM);
	if (ret) {
		(void)pdi_ctl_close(&ctl);
		return ret;
	}
	...
	printf("pdm_mock_runtime_smoke: passed\n");
	return 0;
}
