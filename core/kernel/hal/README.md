...
HAL (Hardware Abstraction Layer) 封装 Linux 硬件访问细节，为内核态 PDM 提供统一的硬件访问接口。

**设计理念**：
- 唯一允许包含硬件访问细节的层
- 面向 Linux 内核态 PDM，不再承担 RTOS/裸机兼容目标
- 必须通过 kernel OSAL 或 Linux kernel 标准接口访问系统能力
- 硬件抽象层设计，隔离 PDM 与具体硬件访问方式
...
- **驱动隔离**：硬件相关代码按驱动类型组织在 `src/` 下
- **统一接口**：提供 Linux 硬件访问 API
- **驱动封装**：CAN、串口等硬件驱动的高层封装
- **配置管理**：硬件配置集中在`include/config/`目录
- **OSAL依赖**：必须使用OSAL封装的系统调用，不直接调用系统API

当前 `make modules` 路径只提供 CAN/Serial 的 kernel-safe stub，用于让
PDM MCU 先完成 ko 链接边界；stub 运行时返回 `OSAL_ERR_NOT_SUPPORTED`。
真实 SocketCAN/TTY 内核传输实现仍需后续补齐。
...
```bash
# 在项目根目录使用统一构建命令
make tests_x86_full_defconfig  # 或其他包含 HAL 的配置
make

# 或使用 ARM64 HAL 测试配置
make tests_arm64_hal_defconfig
make
```
...
3. 在 `src/<driver>/` 添加实现（如 `src/gpio/hal_gpio_linux.c`）
...
1. 创建新的运行域目录（如后续内核态可使用 `src/kernel/`）
2. 实现HAL接口（参考现有 `src/<driver>/` 实现并按内核态约束改造）
3. 修改 `CMakeLists.txt` 添加平台判断：
   ```cmake
   elseif(CONFIG_HAL_KERNEL_BACKEND)
       set(HAL_SOURCES
           src/kernel/hal_can.c
           src/kernel/hal_serial.c
       )
   ```
4. 运行测试验证移植
...
# 2. 实现驱动
cp hal/src/can/hal_can_linux.c hal/src/my_backend/
...
当前支持的平台：
- **Linux user space** (`src/`) - 基于 SocketCAN、sysfs、i2c-dev、spidev、termios
