...
AConfig (Application Control Layer) 提供应用配置和控制接口。
...
**说明**: 必须在使用其他 AConfig 函数之前调用。

**示例**:
```c
int32_t ret = ACONFIG_Init();
if (ret != OSAL_SUCCESS) {
    printf("Failed to initialize AConfig\n");
    return -1;
}
```
...
```c
// 定义遥控配置表
static const aconfig_tc_config_t tc_configs[] = {
    {
...
// 定义遥测配置表
static const aconfig_tm_config_t tm_configs[] = {
    {
...
static const aconfig_invalidation_map_t inv_maps[] = {
    {
...
int32_t ret = ACONFIG_RegisterTable(&config_table);
if (ret != OSAL_SUCCESS) {
...
**示例**:
```c
const aconfig_tm_config_t *config = ACONFIG_GetTmConfig(TM_CPU_TEMP);
if (config != NULL) {
    printf("TM Config:\n");
...
**示例**:
```c
if (ACONFIG_IsTmEnabled(TM_CPU_TEMP)) {
    // 读取 CPU 温度
...
### ACONFIG_GetInvalidationMap

获取失效映射。

```c
int32_t ACONFIG_GetInvalidationMap(
...
int32_t ret = ACONFIG_GetInvalidationMap(TM_POWER_STATUS, 
...
**示例**:
```c
const aconfig_tc_config_t *config = ACONFIG_GetTcConfig(TC_POWER_ON);
if (config != NULL) {
    printf("TC Config:\n");
    printf("  Device Type: %d\n", config->device_type);
    printf("  Logic Index: %u\n", config->logic_index);
...
**示例**:
```c
if (ACONFIG_IsTcEnabled(TC_POWER_ON)) {
    // 执行开机命令
...
```c
typedef enum {
    ACONFIG_DEVICE_SATELLITE = 0,  /* 卫星设备 */
...
// ❌ 错误 - 使用动态分配
aconfig_tc_config_t *tc_configs = malloc(sizeof(aconfig_tc_config_t) * 10);
```
