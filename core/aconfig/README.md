# AConfig层（Application Configuration Layer）

## 概述

AConfig（Application Configuration Layer，业务配置层）是连接业务逻辑和硬件实现的桥梁，通过配置映射将抽象的业务功能映射到具体的硬件设备。
...
├── config/
│   └── pmc_v1/
│       ├── aconfig_pmc_v1.c                # PMC v1.0配置
│       └── aconfig_pmc_v1_invalidation.c   # 遥控命令失效映射
└── CMakeLists.txt
```
...
```c
typedef struct {
    aconfig_tc_config_t tc_table[TC_FUNC_MAX];
    aconfig_tm_config_t tm_table[TM_FUNC_MAX];
} aconfig_lookup_table_t;
```
...
```c
const aconfig_tc_config_t* ACONFIG_GetTcConfig(pmc_tc_function_t function);
const aconfig_tm_config_t* ACONFIG_GetTmConfig(pmc_tm_function_t function);
```

### 检查使能状态

```c
bool ACONFIG_IsTcEnabled(pmc_tc_function_t function);
bool ACONFIG_IsTmEnabled(pmc_tm_function_t function);
```

### 遥测缓存失效

```c
void ACONFIG_InvalidateAffectedTelemetry(pmc_tc_function_t tc_function);
```

### 配置验证

```c
int32_t ACONFIG_ValidateConfig(void);
```

### 统计与调试

```c
void ACONFIG_GetStatistics(aconfig_statistics_t *stats);
void ACONFIG_PrintStatistics(void);
void ACONFIG_DumpConfig(void);
```
...
int32_t handle_telecommand(pmc_tc_function_t cmd_type, uint32_t param)
{
    ...
    if (ret == OSAL_SUCCESS) {
        ACONFIG_InvalidateAffectedTelemetry(cmd_type);
    }
...
const aconfig_tc_config_t g_pmc_tc_configs[] = {
    /* 服务器电源控制 → BMC */
    { TC_SERVER_POWER_ON,  ACONFIG_DEVICE_BMC, 0, true },
    { TC_SERVER_POWER_OFF, ACONFIG_DEVICE_BMC, 0, true },
    
    /* MCU控制 → MCU[0] */
    { TC_MCU_RESET,        ACONFIG_DEVICE_MCU, 0, true },
};

const aconfig_tm_config_t g_pmc_tm_configs[] = {
    /* 服务器遥测（缓存型） → BMC，1秒更新，2秒有效期 */
    { TM_SERVER_CPU_TEMP,  ACONFIG_DEVICE_BMC, 0, TM_TYPE_CACHED, 2000, 1000, 0, true },
    
    /* 服务器遥测（实时型） → BMC，100ms有效期，1ms超时 */
    { TM_SERVER_POWER_STATUS, ACONFIG_DEVICE_BMC, 0, TM_TYPE_REALTIME, 100, 0, 1000, true },
};
```
...
```cmake
set(ACONFIG_CONFIG "pmc_v1" CACHE STRING "AConfig configuration")
```
...
5. **线程安全**：ACONFIG_Init()必须在单线程环境下调用，之后的查询接口线程安全
