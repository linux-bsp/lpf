...
} pconfig_interface_type_t;

/* 每个外设包含接口配置联合体 */
typedef struct {
    pconfig_interface_type_t interface_type;
    union {
        pconfig_can_cfg_t  can;
        pconfig_uart_cfg_t uart;
        pconfig_i2c_cfg_t  i2c;
        pconfig_spi_cfg_t  spi;
        pconfig_ethernet_cfg_t ethernet;
    } interface_cfg;
} pconfig_interface_t;
```
...
typedef struct {
    ...
    /* GPIO控制 */
    pconfig_gpio_config_t *reset_gpio;
    pconfig_gpio_config_t *irq_gpio;
} pconfig_mcu_cfg_t;
```
...
typedef struct {
    ...
    /* 主通道（以太网） */
    struct {
        pconfig_interface_type_t type;
        pconfig_ethernet_cfg_t cfg;
    } primary_channel;
    
    /* 备份通道（串口） */
    struct {
        pconfig_interface_type_t type;
        pconfig_uart_cfg_t cfg;
    } backup_channel;
    ...
    /* GPIO控制 */
    pconfig_gpio_config_t *power_gpio;
    pconfig_gpio_config_t *reset_gpio;
} pconfig_bmc_cfg_t;
```
...
typedef struct {
    ...
    /* 外设配置列表（以外设为单位） */
    pconfig_mcu_cfg_t **mcus;
    uint32_t mcu_count;
    
    pconfig_bmc_cfg_t **bmcs;
    uint32_t bmc_count;
    
    pconfig_satellite_cfg_t **satellites;
    uint32_t satellite_count;
    
    pconfig_sensor_cfg_t **sensors;
    uint32_t sensor_count;
    
    pconfig_storage_cfg_t **storages;
    uint32_t storage_count;
} pconfig_board_config_t;
```
...
{
    /* TI AM6254平台 */
    HW_Config_Register(&pconfig_h200_100p_base);
    HW_Config_Register(&pconfig_h200_100p_v1);
    HW_Config_Register(&pconfig_h200_100p_v2);
    
    /* 演示平台 */
    HW_Config_Register(&pconfig_demo_base);
    HW_Config_Register(&pconfig_demo_v1);
    HW_Config_Register(&pconfig_demo_v2);
}

/* 选择配置 */
const pconfig_board_config_t* HW_Config_SelectDefault(void)
{
    /* 1. 检查环境变量 */
