...
int main(void)
{
    ...
    /* 选择配置 */
    const pconfig_board_config_t *board = HW_Config_SelectDefault();
...
```c
/* 获取板级配置 */
const pconfig_board_config_t *board = HW_Config_GetBoard();

/* 查找MCU外设配置 */
const pconfig_mcu_cfg_t *mcu_cfg = XCONFIG_HW_FindMCU(board, "stm32_mcu");
if (mcu_cfg == NULL) {
...
```c
/* 查找BMC外设配置 */
const pconfig_bmc_cfg_t *bmc_cfg = XCONFIG_HW_FindBMC(board, "payload_bmc");
...
for (uint32_t i = 0; i < board->sensor_count; i++) {
    const pconfig_sensor_cfg_t *sensor = board->sensors[i];
...
```c
/* 新增MCU外设 */
static pconfig_mcu_cfg_t mcu_new = {
    .name = "new_mcu",
    .enabled = true,
...
```c
static pconfig_mcu_cfg_t *mcu_list[] = {
    &mcu_stm32,
    &mcu_new  /* 添加新MCU */
};
```
...
```c
const pconfig_board_config_t pconfig_xxx = {
    /* ... */
    .mcus = mcu_list,
...
完整的使用示例请参考 `pcl/examples/pconfig_example.c`。
