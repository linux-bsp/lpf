...
1. **零拷贝传递** - PConfig → PDL → HAL 无需配置转换
...
```c
// 定义 MCU 配置
static pconfig_mcu_entry_t power_mcu_entry = {
    .name = "power_mcu",
...
// 定义 BMC 配置
static pconfig_bmc_entry_t payload_bmc_entry = {
    .name = "payload_bmc",
...
// 定义板级配置
static pconfig_mcu_entry_t *mcu_array[] = {
    &power_mcu_entry,
    NULL  // 数组结束标记
};

static pconfig_bmc_entry_t *bmc_array[] = {
    &payload_bmc_entry,
    NULL
};

static pconfig_platform_config_t platform_config = {
    .platform_name = "ti",
    .chip_name = "am6254",
...
int main(void) {
    ...
    // 注册平台配置
    PCONFIG_Register(&platform_config);
...
void init_bmc(void) {
    ...
    // 2. 查找 BMC 配置条目
    const pconfig_bmc_entry_t *entry = PCONFIG_HW_FindBMC(platform, "payload_bmc");
    if (!entry || !entry->enabled) {
...
```c
// PConfig 配置 → PDL 配置 → HAL 配置（三次拷贝）
pconfig_mcu_cfg_t pconfig_cfg;      // PConfig 层定义
mcu_config_t pdl_cfg;        // PDL 层定义
hal_can_config_t hal_cfg;    // HAL 层定义

// 需要转换代码
convert_pcl_to_pdl(&pconfig_cfg, &pdl_cfg);
...
```c
// PConfig 条目 → PDL 配置 → HAL 配置（零拷贝）
pconfig_mcu_entry_t entry;       // PConfig 层容器
...
3. **类型安全** - 编译期检查
4. **易维护** - 修改配置只需改一处
5. **清晰职责** - PConfig 只做配置管理，不定义配置结构

### 各层职责

- **HAL**: 定义硬件接口配置（纯硬件参数）
- **PDL**: 定义外设配置（嵌入 HAL 字段 + 业务配置）
- **PConfig**: 配置容器（引用 PDL 配置 + 管理字段）

### 使用建议

1. 新增外设时，在 PDL 层定义配置结构
2. PDL 配置直接嵌入 HAL 字段，避免重复定义
3. PConfig 层只添加配置管理字段（name/enabled/description）
4. 应用层通过 PConfig API 查询配置，直接传递给 PDL
