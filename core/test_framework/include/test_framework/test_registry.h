 ...
 * @return 测试用例结构体
 */
static inline test_case_t test_case_create(const char *name, test_func_t func,
										   fixture_func_t setup,
										   fixture_func_t teardown)
{
	test_case_t test_case = {
		.name = name, .func = func, .setup = setup, .teardown = teardown
	};
	return test_case;
}
 ...
 * @return 测试套件结构体
 */
static inline test_suite_t
test_suite_create(const char *suite_name, const char *module_name,
				  const char *layer_name, const test_case_t *cases,
				  uint32_t case_count, fixture_func_t suite_setup,
				  fixture_func_t suite_teardown, test_metadata_t metadata)
{
	test_suite_t suite = { .suite_name = suite_name,
...
{
	test_metadata_t metadata = { .category = category,
								 .tags = tags,
...
uint32_t test_get_suites_by_category(test_category_t category,
									 const test_suite_t **suites,
									 uint32_t max_suites);
uint32_t test_get_suites_by_tags(uint32_t tags, const test_suite_t **suites,
								 uint32_t max_suites);
