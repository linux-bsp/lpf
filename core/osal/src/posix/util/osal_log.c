...
static void rotate_log_file(void)
{
	...
	/* 删除最旧的日志文件 */
	snprintf(old_file, OSAL_sizeof(old_file), "%s.%u", g_log_file_path,
			 max_files);
	...
	/* 重命名当前日志文件 */
	snprintf(current_backup, OSAL_sizeof(current_backup), "%s.1",
			 g_log_file_path);
...
static void send_remote_log(const char *log_message)
{
	...
	/* 发送日志（不持有锁，避免阻塞） */
	sendto(sock, log_message, strlen(log_message), 0, (struct sockaddr *)&addr,
		   OSAL_sizeof(addr));
}
						 ...
						 uint32_t kv_count)
{
	...
	/* 构造完整消息 */
	snprintf(full_message, OSAL_sizeof(full_message), "%s%s", message,
			 kv_buffer);
