...
int32_t HAL_SPI_open(const hal_spi_config_t *config, hal_spi_handle_t *handle)
{
    ...
    OSAL_snprintf(lock_file,
                  OSAL_sizeof(lock_file),
                  HAL_SPI_LOCK_PATH_FMT,
                  dev_name);
    ...
    if (ret < 0) {
        int32_t err = OSAL_get_errno();
        LOG_ERROR("HAL_SPI",
                  "Failed to set bits per word: %s (%d)",
                  OSAL_strerror(err),
                  err);
        OSAL_close(impl->fd);
        OSAL_pthread_mutex_destroy(&impl->mutex);
...
HAL_SPI_write(hal_spi_handle_t handle, const uint8_t *buffer, uint32_t size)
{
    ...
    /* 第一层：文件锁（进程间保护） */
    ret = OSAL_flock_timed_lock(impl->flock,
                                OSAL_FLOCK_EXCLUSIVE,
                               ...
                               uint32_t num)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        xfers[i].speed_hz = (transfers[i].speed_hz != 0) ? transfers[i].speed_hz
                                                         : impl->max_speed_hz;
                           ...
                           const hal_spi_config_t *config)
{
    ...
    if (config->bits_per_word != impl->bits_per_word) {
        ...
        if (ret < 0) {
            int32_t err = OSAL_get_errno();
            LOG_ERROR("HAL_SPI",
                      "Failed to update bits per word: %s (%d)",
                      OSAL_strerror(err),
                      err);
            result = err;
            goto unlock;
    ...
    if (config->max_speed_hz != impl->max_speed_hz) {
        ...
        if (ret < 0) {
            int32_t err = OSAL_get_errno();
            LOG_ERROR("HAL_SPI",
                      "Failed to update max speed: %s (%d)",
                      OSAL_strerror(err),
                      err);
            result = err;
            goto unlock;
