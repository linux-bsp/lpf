                        ...
                        hal_serial_handle_t *handle)
{
    ...
    ctx->fd =
        OSAL_open(device,
                  OSAL_O_RDWR | OSAL_O_NOCTTY | OSAL_O_NONBLOCK | OSAL_O_EXCL,
                  0);
    if (ctx->fd < 0) {
                         ...
                         int32_t timeout)
{
    ...
    /* 第一层：文件锁（进程间保护） */
    ret = OSAL_flock_timed_lock(ctx->flock,
