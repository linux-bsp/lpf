...
/**
 * @brief 初始化 CCM CAN 通信（适配 ops 接口）
 */
static int32_t ccm_can_init_ops(const void *config, void **handle)
{
	const pconfig_ccm_can_config_t *can_cfg = (const pconfig_ccm_can_config_t *)config;
	ccm_can_context_t *ctx;
	...
	ctx = (ccm_can_context_t *)OSAL_malloc(OSAL_sizeof(ccm_can_context_t));
	if (NULL == ctx)
	{
		return OSAL_ERR_NO_MEMORY;
	}

	OSAL_memset(ctx, 0, OSAL_sizeof(ccm_can_context_t));
	...
	*handle = ctx;
	...
	LOG_INFO("PDL_CCM", "CAN initialized: %s", can_cfg->device);
	return OSAL_SUCCESS;
}

/**
 * @brief 反初始化 CCM CAN 通信
 */
static int32_t ccm_can_deinit(void *handle)
{
	ccm_can_context_t *ctx;
 ...
 * @brief 发送 CCM CAN 消息
 */
static int32_t ccm_can_send(void *handle, const ccm_msg_t *msg, uint32_t timeout_ms)
{
	ccm_can_context_t *ctx;
 ...
 * @brief 接收 CCM CAN 消息
 */
static int32_t ccm_can_recv(void *handle, ccm_msg_t *msg, uint32_t timeout_ms)
{
	ccm_can_context_t *ctx;
	hal_can_frame_t frame;
	uint32_t offset;
 ...
 * @brief 检查 CAN 连接状态
 */
static int32_t ccm_can_is_connected(void *handle)
{
	ccm_can_context_t *ctx;
...
/*
 * CAN 接口的 ops 结构定义（导出供 pdl_ccm.c 使用）
 */
const pdl_ccm_ops_t ccm_can_ops = {
	.init = ccm_can_init_ops,
	.deinit = ccm_can_deinit,
	.send = ccm_can_send,
	.recv = ccm_can_recv,
	.is_connected = ccm_can_is_connected,
};
