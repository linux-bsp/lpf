...
`lpf_configs.ko` module through `g_lpf_config_platform_table`. Individual
static board-description source files register themselves into a linker
section with `lpf_config_static_register()`, so adding a product config does
not require editing a central extern table. The config module only exports
data; it does not register devices and does not depend on `lpf_core.ko`.
...
The static config module exports the section-built table:
...
```text
kernel/lpf-configs/configs/<product>/<project>/lpf_config_<product>_<project>_vN.c
```
...
- `kernel/lpf-configs/configs` owns concrete static platform tables. Each
  concrete config source must use `lpf_config_static_register()` instead of
  adding product-specific externs or `#ifdef CONFIG_LPF_CONFIG_*` branches to
  the common `lpf_configs.ko` table builder.
