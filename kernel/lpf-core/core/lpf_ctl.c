...
#define LPF_CTL_VERSION_MINOR 0x00
#define LPF_CTL_VERSION_PATCH 0x00

static lpf_chrdev_t g_lpf_ctl_chrdev;

static uint32_t lpf_ctl_device_count(void)
{
	uint32_t count = 0;
	int32_t ret;
...
static uint32_t lpf_ctl_device_type(lpf_device_type_t type)
{
	switch (type) {
...
static uint32_t lpf_ctl_device_state(lpf_device_state_t state)
{
	switch (state) {
...
static void lpf_ctl_fill_device_info(const lpf_device_info_t *src,
				     struct lpf_ctl_device_info *dst)
{
	osal_memset(dst, 0, sizeof(*dst));
	dst->type = lpf_ctl_device_type(src->type);
	dst->index = src->index;
	dst->state = lpf_ctl_device_state(src->state);
...
static long lpf_ctl_ioctl_get_info(unsigned long arg)
{
	struct lpf_ctl_info info;

	osal_memset(&info, 0, sizeof(info));
	info.abi_version = LPF_CTL_ABI_VERSION;
	info.module_version_major = LPF_CTL_VERSION_MAJOR;
	info.module_version_minor = LPF_CTL_VERSION_MINOR;
	info.module_version_patch = LPF_CTL_VERSION_PATCH;
	info.open_count = lpf_chrdev_open_count(&g_lpf_ctl_chrdev);
	info.device_count = lpf_ctl_device_count();
...
static long lpf_ctl_ioctl_get_device(unsigned long arg)
{
	...
	if (query.match_index >= lpf_ctl_device_count())
		return -ENOENT;
	...
	lpf_ctl_fill_device_info(&infos[query.match_index], &query.info);
	osal_free(infos);
...
static long lpf_ctl_ioctl_get_device_by_name(unsigned long arg)
{
	...
	lpf_ctl_fill_device_info(&info, &query.info);
	if (osal_copy_to_user((void __user *)arg, &query, sizeof(query)) !=
	    OSAL_SUCCESS)
		return -EFAULT;
...
static long lpf_ctl_ioctl_get_device_by_capability(unsigned long arg)
{
	struct lpf_ctl_device_query query;
	lpf_device_info_t info;
	int32_t ret;
...
static long lpf_ctl_ioctl(struct file *file, unsigned int cmd,
			  unsigned long arg)
{
	(void)file;
	...
	case LPF_CTL_IOC_GET_INFO:
		return lpf_ctl_ioctl_get_info(arg);
	case LPF_CTL_IOC_GET_DEVICE:
		return lpf_ctl_ioctl_get_device(arg);
	case LPF_CTL_IOC_GET_DEVICE_BY_NAME:
		return lpf_ctl_ioctl_get_device_by_name(arg);
	case LPF_CTL_IOC_GET_DEVICE_BY_CAPABILITY:
		return lpf_ctl_ioctl_get_device_by_capability(arg);
	default:
		return -ENOTTY;
	}
}
...
static int lpf_ctl_release(struct inode *inode, struct file *file)
{
	(void)inode;
...
static const struct file_operations g_lpf_ctl_fops = {
	.owner = THIS_MODULE,
	.open = lpf_ctl_open,
	.release = lpf_ctl_release,
	.unlocked_ioctl = lpf_ctl_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = lpf_ctl_compat_ioctl,
#endif
};

int32_t lpf_ctl_chrdev_register(void)
{
	int ret;

	ret = lpf_chrdev_register(&g_lpf_ctl_chrdev, LPF_CTL_DEVICE_NAME,
				  &g_lpf_ctl_fops);
	if (ret == 0)
		return OSAL_SUCCESS;
...
void lpf_ctl_chrdev_unregister(void)
{
	lpf_chrdev_unregister(&g_lpf_ctl_chrdev);
}
