...
typedef struct {
	...
	void *user_data;
	bool enabled;
} pdm_compat_gpio_irq_t;

static unsigned long pdm_compat_gpio_irq_flags(pdm_gpio_edge_t edge)
{
	switch (edge) {
...
static irqreturn_t pdm_compat_gpio_irq_thread(int irq, void *data)
{
	pdm_compat_gpio_irq_t *ctx = data;
	...
	level = value ? PDM_GPIO_LEVEL_HIGH : PDM_GPIO_LEVEL_LOW;
...
{
	return pdm_compat_errno_to_status(
				      ...
				      pdm_gpio_level_t initial_level)
{
	...
	switch (direction) {
	case PDM_GPIO_DIR_INPUT:
		ret = gpio_direction_input(gpio_num);
		break;
	case PDM_GPIO_DIR_OUTPUT:
...
int32_t pdm_compat_gpio_set_level(uint32_t gpio_num, pdm_gpio_level_t level)
{
	if (level != PDM_GPIO_LEVEL_LOW && level != PDM_GPIO_LEVEL_HIGH)
		return OSAL_ERR_INVALID_PARAM;

	gpio_set_value(gpio_num, level == PDM_GPIO_LEVEL_HIGH);
	return OSAL_SUCCESS;
}

int32_t pdm_compat_gpio_get_level(uint32_t gpio_num, pdm_gpio_level_t *level)
{
	int value;
	...
	void **irq_handle)
{
	...
	*irq_handle = NULL;
	flags = pdm_compat_gpio_irq_flags(edge);
	if (flags == ULONG_MAX || flags == 0)
		return OSAL_ERR_INVALID_PARAM;

	irq = gpio_to_irq(gpio_num);
	if (irq < 0)
		return pdm_compat_errno_to_status(irq);
	...
	ret = request_threaded_irq(irq, NULL, pdm_compat_gpio_irq_thread,
...
{
	pdm_compat_gpio_irq_t *ctx = irq_handle;
