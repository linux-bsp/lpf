...
static pdm_debugfs_entry_t g_lpf_led_debugfs;
...
static const char *pdm_led_control_name(pdm_config_led_control_t control)
{
	switch (control) {
...
out:
	if (ret == 0)
		pdm_led_chrdev_record_recovery(index);
...
{
	return pdm_debugfs_register(&g_lpf_led_debugfs, "pdm", "led",
				    pdm_led_proc_write, NULL);
}

void pdm_led_debugfs_unregister(void)
{
	pdm_debugfs_unregister(&g_lpf_led_debugfs);
}
