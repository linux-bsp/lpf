...
static bool pdm_led_ioctl_is_runtime_cmd(unsigned int cmd)
{
	switch (cmd) {
...
static void pdm_led_record_file_result(struct file *file, unsigned int cmd,
				       long ret)
{
	if (!pdm_led_ioctl_is_runtime_cmd(cmd))
		return;

	if (ret == 0)
		pdm_chrdev_record_recovery(pdm_led_chrdev_from_file(file));
...
static uint32_t pdm_led_open_count(void)
{
	...
	for (i = 0; i < OSAL_ARRAY_SIZE(g_lpf_led_chrdevs); i++)
		open_count += pdm_chrdev_open_count(&g_lpf_led_chrdevs[i]);

	return open_count;
}

static void pdm_led_fill_info(struct pdm_led_info *info)
{
	osal_memset(info, 0, sizeof(*info));
	info->abi_version = PDM_LED_ABI_VERSION;
	info->module_version_major = PDM_LED_SERVICE_VERSION_MAJOR;
	info->module_version_minor = PDM_LED_SERVICE_VERSION_MINOR;
	info->module_version_patch = PDM_LED_SERVICE_VERSION_PATCH;
			  ...
			  unsigned long arg)
{
	...
	pdm_led_record_file_result(file, cmd, ret);
	return ret;
}
...
void pdm_led_chrdev_unregister(void)
{
	...
	for (i = 0; i < OSAL_ARRAY_SIZE(g_lpf_led_chrdevs); i++)
		pdm_chrdev_unregister(&g_lpf_led_chrdevs[i]);
}
...
int pdm_led_chrdev_register_device(const pdm_device_t *device)
{
	...
	osal_snprintf(name, sizeof(name), "pdm_led%u", index);
	osal_snprintf(nodename, sizeof(nodename), "lpf/led%u", index);

	return pdm_chrdev_register_lpf_device(&g_lpf_led_chrdevs[index],
...
void pdm_led_chrdev_unregister_device(const pdm_device_t *device)
{
	...
	pdm_chrdev_unregister(&g_lpf_led_chrdevs[index]);
}
...
void pdm_led_chrdev_record_error(uint32_t index, int error)
{
	...
	pdm_chrdev_record_error(&g_lpf_led_chrdevs[index], error);
}
...
void pdm_led_chrdev_record_recovery(uint32_t index)
{
	...
	pdm_chrdev_record_recovery(&g_lpf_led_chrdevs[index]);
}
