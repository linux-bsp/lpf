...
static bool pdm_mcu_ioctl_is_runtime_cmd(unsigned int cmd)
{
	switch (cmd) {
...
static void pdm_mcu_record_file_result(struct file *file, unsigned int cmd,
				       long ret)
{
	if (!pdm_mcu_ioctl_is_runtime_cmd(cmd))
		return;

	if (cmd == PDM_MCU_IOC_GET_STATUS && ret == 0)
		return;

	if (ret == 0)
		pdm_chrdev_record_recovery(pdm_mcu_chrdev_from_file(file));
	else if (pdm_errno_is_runtime_error(ret))
...
static uint32_t pdm_mcu_open_count(void)
{
	...
	for (i = 0; i < OSAL_ARRAY_SIZE(g_lpf_mcu_chrdevs); i++)
		open_count += pdm_chrdev_open_count(&g_lpf_mcu_chrdevs[i]);

	return open_count;
}

static void pdm_mcu_fill_info(struct pdm_mcu_info *info)
{
	osal_memset(info, 0, sizeof(*info));
	info->abi_version = PDM_MCU_ABI_VERSION;
	info->module_version_major = PDM_MCU_SERVICE_VERSION_MAJOR;
	info->module_version_minor = PDM_MCU_SERVICE_VERSION_MINOR;
	info->module_version_patch = PDM_MCU_SERVICE_VERSION_PATCH;
...
{
	struct pdm_mcu_version request;
...
static long pdm_mcu_ioctl_get_status(struct file *file, unsigned long arg)
{
	...
	pdm_mcu_chrdev_record_status(request.index, &status);
...
{
	struct pdm_mcu_command request;
	pdm_mcu_handle_t handle;
...
static long pdm_mcu_ioctl_write_data(struct file *file, unsigned long arg)
{
	...
	if (request.len > PDM_MCU_MAX_WRITE_SIZE)
		return -EINVAL;
			  ...
			  unsigned long arg)
{
	...
	pdm_mcu_record_file_result(file, cmd, ret);
	return ret;
}
...
void pdm_mcu_chrdev_unregister(void)
{
	...
	for (i = 0; i < OSAL_ARRAY_SIZE(g_lpf_mcu_chrdevs); i++)
		pdm_chrdev_unregister(&g_lpf_mcu_chrdevs[i]);
}
...
int pdm_mcu_chrdev_register_device(const pdm_device_t *device)
{
	...
	osal_snprintf(name, sizeof(name), "pdm_mcu%u", index);
	osal_snprintf(nodename, sizeof(nodename), "lpf/mcu%u", index);

	return pdm_chrdev_register_lpf_device(&g_lpf_mcu_chrdevs[index],
...
void pdm_mcu_chrdev_unregister_device(const pdm_device_t *device)
{
	...
	pdm_chrdev_unregister(&g_lpf_mcu_chrdevs[index]);
}
...
void pdm_mcu_chrdev_record_error(uint32_t index, int error)
{
	...
	pdm_chrdev_record_error(&g_lpf_mcu_chrdevs[index], error);
}
...
void pdm_mcu_chrdev_record_recovery(uint32_t index)
{
	...
	pdm_chrdev_record_recovery(&g_lpf_mcu_chrdevs[index]);
}
				  ...
				  const pdm_mcu_status_t *status)
{
	...
	if (!status->online || status->state == PDM_MCU_STATE_OFFLINE) {
		pdm_chrdev_record_error(&g_lpf_mcu_chrdevs[index], -ENODEV);
		return;
	}

	if (status->state == PDM_MCU_STATE_ERROR) {
		pdm_chrdev_record_error(&g_lpf_mcu_chrdevs[index], -EIO);
		return;
	}
