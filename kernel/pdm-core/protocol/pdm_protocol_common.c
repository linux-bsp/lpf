...
#include "pdm_protocol_internal.h"

/* Static storage duration leaves the atomic initialized to zero. */
static osal_atomic_uint32_t g_lpf_protocol_seq_number;
...
{
	return osal_atomic_fetch_add(&g_lpf_protocol_seq_number, 1);
}

void pdm_protocol_reset_sequence(uint32_t seq)
{
	osal_atomic_store(&g_lpf_protocol_seq_number, seq);
}

uint32_t pdm_protocol_get_current_sequence(void)
{
	return osal_atomic_load(&g_lpf_protocol_seq_number);
}
