...
#include "pdm/core/diag/pdm_debugfs.h"
...
typedef struct {
	...
	char name[PDM_DEBUGFS_ROOT_NAME_LEN];
	struct dentry *root;
	uint32_t users;
	bool available;
} pdm_debugfs_root_t;
...
static pdm_debugfs_root_t *
pdm_debugfs_find_root_locked(const char *root_name)
{
...
EXPORT_SYMBOL_GPL(pdm_debugfs_unregister);
