 ...
 * - 配置类型由 PDM_CONFIG 定义，PDM MCU service 使用
 ************************************************************************/

#ifndef PDM_MCU_SERVICE_H
#define PDM_MCU_SERVICE_H

#include "osal.h"
#include "pdm/pdm_mcu.h"

#define PDM_MCU_SERVICE_VERSION_MAJOR 0x01
#define PDM_MCU_SERVICE_VERSION_MINOR 0x00
#define PDM_MCU_SERVICE_VERSION_PATCH 0x00
 ...
 * @brief MCU服务句柄
 */
typedef void *pdm_mcu_handle_t;

typedef enum pdm_mcu_state pdm_mcu_state_t;
 ...
 * @brief 初始化MCU驱动
 *
 * @param[in] index MCU设备索引（从 PDM_CONFIG 获取配置）
 ...
 *
 * @note 函数内部会：
 *       1. 调用 pdm_config_get_board() 获取平台配置
 *       2. 调用 pdm_config_hw_get_mcu(platform, index) 获取 MCU 配置
 *       3. 检查配置是否启用
 *       4. 将 PDM_CONFIG 配置转换为 PDM HW 配置并初始化硬件
 */
pdm_mcu_handle_t pdm_mcu_get(uint32_t index);
int32_t pdm_mcu_init(uint32_t index, pdm_mcu_handle_t *handle);
 ...
 * @return OSAL_SUCCESS 成功
 */
int32_t pdm_mcu_get_version(pdm_mcu_handle_t handle,
 ...
 * @return OSAL_SUCCESS 成功
 */
int32_t pdm_mcu_send_command(pdm_mcu_handle_t handle, uint8_t cmd,
