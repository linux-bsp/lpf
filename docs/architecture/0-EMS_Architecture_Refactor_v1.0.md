...
5. **外设固件升级**：接收载荷服务器通过以太网传输的外设固件，升级MCU/FPGA等外设

6. **日志收集**：收集服务器状态、外设信息、进程日志、崩溃日志
...
**业务场景**：载荷服务器通过以太网传输外设固件到AM625，然后AM625升级MCU/FPGA等外设。

**进程架构**：新增第6个进程 `pmc_fwupdate`（按需启动）

```text
┌─────────────────────────────────────────────────────────────────────┐
│                      载荷服务器（Payload Server）                     │
│                                                                      │
│  • 外设固件存储在硬盘                                                 │
│  • 运行固件传输服务进程                                               │
│  • 通过以太网发送固件到AM625                                          │
...
│                                                                      │
│  ┌────────────────────────────────────────────────────────────┐    │
│  │  pmc_fwupdate进程（按需启动，CPU2/3）                       │    │
│  │                                                            │    │
│  │  职责：                                                     │    │
│  │  1. 监听TCP端口（默认8888）                                 │    │
│  │  2. 接收固件文件（分块传输）                                │    │
│  │  3. 校验完整性（CRC32/MD5）                                 │    │
│  │  4. 存储到临时目录（/tmp/firmware/）                        │    │
│  │  5. 调用PDL接口升级外设                                     │    │
│  │  6. 返回升级结果                                            │    │
│  │  7. 清理临时文件                                            │    │
│  │                                                            │    │
...
│  ┌────────────────────────────────────────────────────────────┐    │
│  │  PDL层（外设驱动层）                                        │    │
│  │  • PDL_MCU_FirmwareUpdate()  - MCU固件升级                 │    │
│  │  • PDL_FPGA_FirmwareUpdate() - FPGA固件升级                │    │
│  └────────────────────────────────────────────────────────────┘    │
│                           │                                         │
│                           ↓                                         │
│                    外设（MCU/FPGA）                                  │
...
// 命令类型
#define FW_CMD_START      0x01  // 开始传输（包含文件名、大小、MD5）
#define FW_CMD_DATA       0x02  // 数据块传输（最大64KB）
...
```bash
/tmp/firmware/              # 临时固件目录
├── mcu_fw_v1.2.3.bin      # MCU固件
├── fpga_fw_v2.0.1.bit     # FPGA固件
└── .md5sum                # MD5校验文件
```
...
```c
// Supervisor进程：按需启动pmc_fwupdate
int32_t start_firmware_update_service(void) {
    pid_t pid = fork();
    if (pid == 0) {
        // 子进程：启动固件升级服务
        execl("/usr/bin/pmc_fwupdate", "pmc_fwupdate", NULL);
        exit(1);
    }
    
    LOG_INFO("SUPERVISOR", "固件升级服务已启动，PID=%d", pid);
    return OSAL_SUCCESS;
}
```
