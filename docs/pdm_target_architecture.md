# PDM Target Architecture

## Positioning

PDM is a Linux peripheral business abstraction framework for multi-kernel,
multi-SoC, and multi-product deployments. The framework keeps a stable
upper-layer peripheral model and pushes Linux kernel, SoC, and vendor BSP
differences into lower adapter layers.

PDM does not use the Linux native device model as its only internal center.
Linux device nodes, sysfs, debugfs, procfs, and kernel subsystem APIs are
integration surfaces below or beside the PDM-owned model.

## Target Stack

```text
Application / Product
        ↓
ACONFIG
        ↓
PDI - userspace SDK
        ↓
PDM UAPI / ioctl
        ↓
PDM Peripheral Service Layer
        ↓
PDM Core
        ↓
PDM HW API
        ↓
PDM SoC Adapter Layer
        ↓
PDM Kernel Compat Layer
        ↓
Linux Kernel / Vendor BSP / Hardware
```

## Layer Responsibilities

- ACONFIG owns userspace application-facing product metadata.
- PDI owns userspace C APIs and hides ioctl details from applications.
- UAPI owns fixed ABI structures, ioctl constants, and ABI version constants.
- PDM Peripheral Services own reusable MCU, LED, and future peripheral business
  behavior, including service-specific transport backends.
- PDM Core owns the PDM device and driver model, lifecycle, discovery, kernel
  event notification, and shared runtime node helpers.
- PDM HW owns framework hardware capability APIs consumed by services.
- PDM SoC Adapter owns SoC and vendor BSP backend differences.
- PDM Kernel Compat owns Linux kernel API-version differences.

## Runtime Boundaries

The current module boundary is:

```text
osal.ko
pdm_configs.ko
pdm_core.ko
```

`pdm_core.ko` hosts the PDM device model, discovery control node, shared
chrdev/sysfs/debugfs/proc helpers, protocol helpers, kernel compat wrappers, and
the selected SoC adapter. It also hosts the integrated runtime: runtime
configuration backends, PDM HW objects, peripheral services, service-owned
transport backends, and configured-device probing.

`pdm_configs.ko` hosts the selected static board description. It is a
DTS-like data provider, does not depend on `pdm_core.ko`, and is loaded before
Core so the static backend can discover its exported table.

Core lifecycle ownership is module-scoped: only `pdm_core.ko` module
initialization and exit create or destroy Core global state. Public Core APIs
such as driver registration, device registration, and event subscription require
initialized Core state. `pdm_core.ko` declares a soft dependency on `osal.ko`;
the recommended load order is `osal.ko`, `pdm_configs.ko`, then
`pdm_core.ko`.

PDM Core matching is type-based in v1: one `pdm_driver_t` owns one
`pdm_device_type_t`, and `pdm_device_register()` binds a configured device to
the registered driver with the same type before calling `probe()`.
Runtime config driver dispatch can optionally refine that selection by node
compatible string before falling back to the type-only config driver.

## Runtime Access Model

- `/dev/pdm_ctl` is the Core-owned control and discovery node.
- `/dev/pdm_ctl` exposes synchronous device snapshots; PDM v1 keeps device
  event notification kernel-only.
- `/dev/pdm/<type><index>` is the per-instance business ABI.
- Per-instance PDM nodes default to `0660` through
  `CONFIG_PDM_INSTANCE_DEVNODE_MODE`; products should assign group ownership
  with udev, devtmpfs policy, or their init system rather than broadening the
  framework default.
- Sysfs attributes are read-only runtime inspection data.
- Debugfs files are debug command channels and are not stable product ABI.
- Procfs files are read-only service status snapshots.

## Extension Model

Adding a new peripheral family should add matching pieces together:

- Runtime config type and backend parsing.
- PDM device type and capability mapping.
- Peripheral service driver registered with PDM Core.
- Optional service-specific transport implementation under the owning
  peripheral directory.
- Optional PDM HW capability if the peripheral needs a reusable hardware API.
- UAPI header when userspace access is needed.
- PDI wrapper when application-facing C APIs are needed.
- Tests covering ABI layout and operation marshaling.
