...
│                         用户空间                                     │
├─────────────────────────────────────────────────────────────────────┤
│  /dev/pdm-led/led0  │  /dev/pdm/mcu0  │  /proc/pdm  │  /sys/bus/pdm│
└──────────────┬──────┴──────────┬───────┴──────┬──────┴──────┬───────┘
               │                 │              │             │
┌──────────────┴─────────────────┴──────────────┴─────────────┴───────┐
│                     字符设备层                                        │
...
- LED 设备：`/dev/pdm-led/led0`, `/dev/pdm-led/led1`, ...
...
```c
int pdm_cdev_register(struct pdm_device *pdm_dev, 
                     const struct file_operations *fops,
                     const char *devnode_fmt, ...)
{
    struct pdm_cdev *cdev;
    dev_t devt;
    
    // 分配设备号
    alloc_chrdev_region(&devt, 0, 1, dev_name(&pdm_dev->dev));
    ...
    // 添加到系统
    cdev_add(&cdev->cdev, devt, 1);
    
    // 创建设备节点（例如 /dev/pdm-led/led0）
    device_create(pdm_class, &pdm_dev->dev, devt, pdm_dev, devnode_fmt, ...);
...
```
/dev/pdm-led/led0 → pdm_led_fops.ioctl → pdm_led_ioctl() → 后端 apply()
...
**代码引用**：`/home/wanguo/workspase/lpf/kernel/pdm/pdm.c:pdm_module_init()`
...
static int pdm_led_probe(struct pdm_device *pdm_dev)
{
    ...
    inst = devm_kzalloc(&pdm_dev->dev, sizeof(*inst), GFP_KERNEL);
    
    // 步骤 2: 初始化基础字段
    inst->base.pdm_dev = pdm_dev;
    pdm_device_set_drvdata(pdm_dev, inst);
    
    // 步骤 3: 查找后端
    const struct pdm_backend_entry *backend_entry;
    backend_entry = pdm_backend_find(PDM_LED_DEVICE_TYPE,
                                    PDM_BACKEND_CLASS_CONTROL,
                                    pdm_dev->compatible);
    ...
    inst->backend_ops = backend_entry->ops;
    
    // 步骤 4: 调用后端 setup
    if (inst->backend_ops->setup) {
        ret = inst->backend_ops->setup(inst);
        if (ret < 0)
            return ret;
    }
    
    // 步骤 5: 创建字符设备
    ret = pdm_cdev_register(pdm_dev, &pdm_led_fops, "pdm-led/led%d", pdm_dev->id);
    if (ret < 0)
        goto err_cleanup_backend;
    
    // 步骤 6: 初始化 LED 状态
    inst->state.brightness = 0;
    inst->state.on = false;
    
    dev_info(&pdm_dev->dev, "LED device registered: /dev/pdm-led/led%d\n",
             pdm_dev->id);
    
    return 0;
    
err_cleanup_backend:
    if (inst->backend_ops->cleanup)
        inst->backend_ops->cleanup(inst);
    return ret;
}
```
...
```
/home/wanguo/workspase/lpf/kernel/pdm/
├── pdm.c                          # 主模块入口
├── Makefile                       # 构建系统
├── Kconfig                        # 内核配置
│
...
```
/home/wanguo/workspase/lpf/kernel/include/pdm/
├── bus/
│   ├── pdm_bus.h                  # 总线接口定义
│   └── pdm_device.h               # 设备结构定义
├── registry/
...
```
/home/wanguo/workspase/lpf/uapi/pdm/
