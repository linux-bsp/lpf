...
PDM UAPI headers under `uapi/pdm/` define the stable ioctl ABI shared by PDM
kernel character devices and userspace PDI wrappers. PDI owns the
application-facing SDK; PDM UAPI owns ioctl commands, fixed-layout payloads,
and ABI version constants.

## Header Rules

- One peripheral owns one UAPI header: `pdm_<peripheral>.h`. Shared management
  or discovery nodes own explicit control headers such as `pdm_ctl.h`.
...
- UAPI structures use Linux fixed-width types such as `__u32`, `__s32`, and
  `__u64`.
- UAPI structures must not expose kernel-only PDM HW, PDM runtime config, PDM Core, PDM
...
- Each peripheral UAPI defines `PDM_<PERIPHERAL>_ABI_VERSION`.
...
- PDM peripheral character device implementation for
  `/dev/pdm/<peripheral><index>`
- `PDM_<PERIPHERAL>_IOC_GET_INFO`
- A unique `PDM_<PERIPHERAL>_IOC_MAGIC`
...
`uapi/pdm/pdm_ctl.h` only for device discovery snapshots.

PDM v1 does not expose asynchronous userspace device events through the control
