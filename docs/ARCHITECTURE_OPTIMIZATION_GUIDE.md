...
#### 1.2 冗余依赖（严重）
**问题**：
- PConfig 声明依赖 HAL，但从不使用
- AConfig 声明依赖 PDL，但从不使用
...
const pconfig_mcu_entry_t *entry = PCONFIG_HW_FindMCU(platform, "power_mcu");
if (entry) {
...
1. products/ccm/configs/acl/ccm_acl_config.c
   ...
   ↓
2. ACONFIG_RegisterTable(&g_pmc_acl_table)
   注册 AConfig 配置
   ↓
3. products/ccm/configs/pcl/pconfig_h200_100p_base.c
   定义：power_mcu → CAN0, ID=0x123
   ↓
4. PCONFIG_Register(&pconfig_h200_100p_base)
   注册 PConfig 配置
   ↓
5. 应用层调用
   ACONFIG_GetTmConfig(TM_POWER_VOLTAGE) → device_name="power_mcu"
   PDL_MCU_InitByName("power_mcu", &handle)
     → 内部调用 PCONFIG_HW_FindMCU("power_mcu")
     → 获取配置并初始化
```
...
### 1. 清晰的职责分离
- **AConfig**：业务功能到设备的映射（TM_POWER_VOLTAGE → "power_mcu"）
- **PConfig**：硬件配置管理（"power_mcu" → CAN0, ID=0x123）
- **PDL**：设备驱动实现（协议封装、硬件操作）

### 2. 易于维护
- 更换硬件：只需修改 PConfig 配置
- 更换设备映射：只需修改 AConfig 配置
- 应用层代码无需修改

### 3. 易于测试
- 可以使用模拟配置测试 PDL
- 可以独立测试 AConfig 和 PConfig

## 迁移指南

### 从旧接口迁移到新接口

**步骤 1**：更新 AConfig 配置，添加 device_name
```c
// 旧配置
[TM_POWER_VOLTAGE] = {
    .device_type = ACONFIG_DEVICE_MCU,
...
// 旧代码
const pconfig_platform_config_t *platform = PCONFIG_GetBoard();
const pconfig_mcu_entry_t *entry = PCONFIG_HW_GetMCU(platform, tm_config->logic_index);
PDL_MCU_Init(&entry->config, &handle);
...
2. **配置顺序**：必须先调用 `PCONFIG_Register()` 再使用 `PDL_*_InitByName()`
3. **设备名称**：device_name 必须与 PConfig 配置中的设备名称一致
4. **错误处理**：`PDL_*_InitByName()` 返回 `OSAL_ERR_NOT_FOUND` 表示设备未找到

## 后续优化建议

### 长期优化（可选）
1. 考虑重命名 PConfig 为 HCL (Hardware Configuration Layer)
2. 创建独立的配置类型模块
3. 定义统一的错误码体系
4. 添加配置验证工具
