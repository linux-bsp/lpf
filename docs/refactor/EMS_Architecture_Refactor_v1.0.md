...
║                          OSAL运行环境（底层平台抽象）                          ║
║  屏蔽硬件平台差异：Linux/RTOS、32位/64位、系统调用封装                         ║
║  提供统一接口：Process/Shm/Mq/Thread/Mutex/Semaphore/Signal/Time              ║
╠═══════════════════════════════════════════════════════════════════════════════╣
║                     所有模块运行在OSAL之上，调用OSAL接口                       ║
║                                                                                ║
║  ┌─────────────────────────────────────────────────────────────────────────┐ ║
║  │                        应用层 (APP Layer)                               │ ║
║  │  ┌──────────────────────────────────────────────────────────────────┐  │ ║
║  │  │  APP进程 (Telecommand/Telemetry/Firmware/Logger)                │  │ ║
║  │  │  • 业务逻辑实现                                                  │  │ ║
║  │  │  • 根据ACL配置决定调用哪个PDL接口                                │  │ ║
║  │  │  • 直接调用PDL API执行外设操作                                   │  │ ║
║  │  └──────────────────────────────────────────────────────────────────┘  │ ║
...
║  │  │  TC_MCU_RESET    → 失效 TM_MCU_STATUS, TM_MCU_TEMP (STALE)      │  │ ║
║  │  └──────────────────────────────────────────────────────────────────┘  │ ║
║  └─────────────────────────────────────────────────────────────────────────┘ ║
║                                                                                ║
║                                    │ 直接调用PDL API                           ║
...
【关键设计说明】
┌────────────────────────────────────────────────────────────────────────────┐
│ ✅ OSAL运行环境: 底层平台抽象，所有模块运行在OSAL之上，调用OSAL接口       │
│ ✅ 横向配置关系: APP读取ACL配置，PDL读取PCL配置（配置层不参与调用链）     │
│ ✅ 纵向调用链: APP → PDL → HAL (每层直接调用下层API)                      │
│ ✅ 完全解耦: 每层独立，单向依赖，接口隔离，无跨层依赖                      │
...
│ ✅ PDL独立设计: 每种外设独立API，ACL层配置指定具体外设类型                │
└────────────────────────────────────────────────────────────────────────────┘
```

## 4. 进程职责详细说明

### 4.1 Supervisor进程（监控进程）
