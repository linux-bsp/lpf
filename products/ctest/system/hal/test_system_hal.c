...
static void test_system_can_loopback(void)
{
    ...
    /* 发送多个测试帧 */
    for (int i = 0; i < 5; i++) {
        hal_can_frame_t tx_frame = { .can_id = 0x100 + i,
                                     .dlc = 4,
                                     .data = { i, i + 1, i + 2, i + 3 } };
...
static void test_system_uart_lifecycle(void)
{
    ...
    ret = HAL_SERIAL_write(handle,
                           (const uint8_t *)test_msg,
                           OSAL_strlen(test_msg),
                           1000);
...
static void test_system_gpio_lifecycle(void)
{
    ...
    /* 阶段2: 输出高电平 */
    ret = HAL_GPIO_set_level(TEST_GPIO_PIN, HAL_GPIO_LEVEL_HIGH);
    TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);

    OSAL_msleep(100); /* 100ms */

    /* 阶段3: 输出低电平 */
    ret = HAL_GPIO_set_level(TEST_GPIO_PIN, HAL_GPIO_LEVEL_LOW);
    TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);

    OSAL_msleep(100); /* 100ms */

    /* 阶段4: 切换为输入模式 */
    ret = HAL_GPIO_set_direction(TEST_GPIO_PIN, HAL_GPIO_DIR_INPUT);
    TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);

    /* 阶段5: 读取电平 */
    hal_gpio_level_t level;
    ret = HAL_GPIO_get_level(TEST_GPIO_PIN, &level);
    TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
...
static void test_system_gpio_toggle(void)
{
    ...
    /* 快速翻转100次 */
    for (int i = 0; i < 100; i++) {
        ret = HAL_GPIO_set_level(TEST_GPIO_PIN,
                                 (i % 2) ? HAL_GPIO_LEVEL_HIGH
                                         : HAL_GPIO_LEVEL_LOW);
        TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
        OSAL_msleep(1); /* 1ms */
    }
...
static void test_system_multi_peripheral_coordination(void)
{
    ...
    /* 初始化CAN */
    hal_can_config_t can_config = { .interface = TEST_CAN_INTERFACE,
                                    .baudrate = TEST_CAN_BAUDRATE,
                                    .rx_timeout = 500,
                                    .tx_timeout = 500 };
    ...
    for (int i = 0; i < 10; i++) {
        /* 操作CAN */
        if (can_handle) {
            hal_can_frame_t frame = { .can_id = 0x200 + i,
                                      .dlc = 2,
                                      .data = { i, i + 1 } };
...
static void test_system_hardware_fault_recovery(void)
{
    ...
    {
        hal_can_handle_t handle = NULL;
        hal_can_config_t config = { .interface = "invalid_can",
                                    .baudrate = TEST_CAN_BAUDRATE,
                                    .rx_timeout = 100,
                                    .tx_timeout = 100 };
        ...
        if (ret == OSAL_SUCCESS) {
            ...
            OSAL_printf(
                "[ SKIP ] CAN device not available for recovery test\n");
        }
    }
    ...
    {
        hal_serial_handle_t handle = NULL;
        hal_serial_config_t config = { .baud_rate = 9999999, /* 无效波特率 */
                                       .data_bits = 8,
                                       .parity = HAL_SERIAL_PARITY_NONE,
                                       .stop_bits = 1,
                                       .flow_control = HAL_SERIAL_FLOW_NONE };
    ...
    OSAL_printf("[ PASS ] Hardware fault recovery test completed (%d scenarios "
                "recovered)\n",
                recovery_count);
}
...
static const test_suite_t test_suite = {
    ...
    .metadata = { .category = TEST_CATEGORY_SYSTEM,
                  ...
                  .description = "HAL system integration tests - lifecycle, "
                                 "coordination, and recovery" }
};

/* 测试套件注册函数 */
__attribute__((constructor)) static void register_system_hal_tests(void)
{
