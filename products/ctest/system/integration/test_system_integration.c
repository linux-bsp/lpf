...
static int32_t create_pty_pair(void)
{
    ...
    ret = OSAL_openpty(&g_pty_master_fd,
                       &g_pty_slave_fd,
                       g_pty_slave_name,
                       NULL,
                       NULL);
...
static void *pty_responder_thread(void *arg)
{
    ...
    while (g_responder_running) {
        ...
        if (encoded_len > 0) {
            (void)OSAL_write(g_pty_master_fd,
                             tx_buffer,
...
static void setup_full_stack(void)
{
    ...
    ret = OSAL_pthread_create(&g_responder_thread,
                              NULL,
                              pty_responder_thread,
                              NULL);
    if (ret != OSAL_SUCCESS) {
...
static void test_full_stack_configuration_path(void)
{
    ...
    ret = PDL_MCU_send_command(handle,
                               0x42,
                               g_test_payload,
                               OSAL_sizeof(g_test_payload),
                               response,
                               OSAL_sizeof(response),
                               &response_len);
    TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
...
__attribute__((constructor)) static void register_system_integration_tests(void)
{
