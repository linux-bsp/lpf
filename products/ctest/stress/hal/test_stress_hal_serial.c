...
static int32_t serial_continuous_tx_worker(void *user_data, uint32_t iteration)
{
    ...
    /* Transmit data */
    ret = HAL_SERIAL_write(ctx->handle,
                           buffer,
                           SERIAL_STRESS_BUFFER_SIZE,
                           &written);
    if (ret == OSAL_SUCCESS) {
...
static void test_stress_serial_continuous_transmission(void)
{
    ...
    OSAL_printf("         Duration: %u sec, Baudrate: %u\n",
                SERIAL_STRESS_DURATION_SEC,
                SERIAL_STRESS_BAUDRATE);
                                             ...
                                             uint32_t iteration)
{
    ...
    /* Try to read data */
    ret = HAL_SERIAL_read(ctx->handle,
                          buffer,
                          SERIAL_STRESS_PATTERN_SIZE,
                          &read_count);
    if (ret == OSAL_SUCCESS) {
...
static void test_stress_serial_concurrent_operations(void)
{
    ...
    /* Start write workers */
    for (uint32_t i = 0; i < SERIAL_STRESS_THREAD_COUNT; i++) {
        ret = stress_run(write_stress,
                         serial_concurrent_write_worker,
                         &write_ctx[i]);
        TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
    }
    ...
    /* Start read workers */
    for (uint32_t i = 0; i < SERIAL_STRESS_THREAD_COUNT; i++) {
        ret = stress_run(read_stress,
                         serial_concurrent_read_worker,
                         &read_ctx[i]);
        TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
    }
...
__attribute__((constructor)) static void register_stress_hal_serial_tests(void)
{
