...
static void test_stress_gpio_high_frequency_toggle(void)
{
    ...
    OSAL_printf("         Pins: %u, Toggles per pin: %u\n",
                GPIO_STRESS_THREAD_COUNT,
                GPIO_STRESS_TOGGLE_COUNT);
                                             ...
                                             uint32_t iteration)
{
    ...
    hal_gpio_level_t expected =
        (iteration % 2) ? HAL_GPIO_LEVEL_HIGH : HAL_GPIO_LEVEL_LOW;
    if (level == expected) {
        OSAL_atomic_inc(ctx->toggle_counter);
    }
...
static void test_stress_gpio_concurrent_access(void)
{
    ...
    stress_context_t *stress_ctx = NULL;
    stress_config_t stress_config =
        STRESS_CONFIG_CONCURRENCY(GPIO_STRESS_PIN_COUNT,
    ...
    /* Run stress test */
    for (uint32_t i = 0; i < GPIO_STRESS_PIN_COUNT; i++) {
        ret = stress_run(stress_ctx,
                         gpio_concurrent_access_worker,
                         &worker_ctx[i]);
        TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
    }
 ...
 */
static void gpio_interrupt_storm_callback(uint32_t pin,
                                          hal_gpio_level_t level,
                                          void *user_data)
{
...
static void test_stress_gpio_interrupt_storm(void)
{
    ...
    for (uint32_t trigger = 0; trigger < triggers_per_pin; trigger++) {
        for (uint32_t i = 0; i < GPIO_STRESS_PIN_COUNT; i++) {
            hal_gpio_level_t level =
                (trigger % 2) ? HAL_GPIO_LEVEL_HIGH : HAL_GPIO_LEVEL_LOW;
...
static void test_stress_gpio_rapid_config_changes(void)
{
    ...
    for (uint32_t i = 0; i < iterations; i++) {
        hal_gpio_direction_t dir =
            (i % 2) ? HAL_GPIO_DIRECTION_INPUT : HAL_GPIO_DIRECTION_OUTPUT;
        ...
        if (dir == HAL_GPIO_DIRECTION_OUTPUT) {
            hal_gpio_level_t level =
                (i % 4 < 2) ? HAL_GPIO_LEVEL_HIGH : HAL_GPIO_LEVEL_LOW;
...
__attribute__((constructor)) static void register_stress_hal_gpio_tests(void)
{
