...
static void *spi_worker_thread(void *arg)
{
    ...
    while (!ctx->stop_flag) {
        ...
        ret = HAL_SPI_transfer(ctx->spi_handle,
                               tx_buffer,
                               rx_buffer,
                               sizeof(tx_buffer));
        if (ret == OSAL_SUCCESS) {
...
static void test_stress_hal_all_drivers_concurrent(void)
{
    ...
    if (ctx.can_handle) {
        OSAL_pthread_create(&threads[thread_idx++],
                            NULL,
                            can_worker_thread,
                            &ctx);
    }
...
static const test_suite_t test_suite = {
    ...
    .metadata = { .category = TEST_CATEGORY_STRESS,
                  .tags =
                      TEST_TAG_SLOW | TEST_TAG_HARDWARE | TEST_TAG_INTEGRATION,
                  .timeout_ms = 180000,
                  .description = "HAL concurrent multi-driver stress tests" }
};

__attribute__((constructor)) static void
register_stress_hal_concurrent_tests(void)
{
