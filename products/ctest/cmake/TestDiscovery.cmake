...
#   test_pdi_api.c          -->  CONFIG_TEST_PDI_API
#
...
function(_test_collect_registration_function TEST_FILE)
    file(READ "${TEST_FILE}" TEST_SOURCE)
    string(REGEX MATCH "(register_[A-Za-z0-9_]+_tests)[ \t\r\n]*\\(" REG_MATCH "${TEST_SOURCE}")

    if(CMAKE_MATCH_1)
        get_property(REG_FUNCS GLOBAL PROPERTY CTEST_REGISTRATION_FUNCTIONS)
        if(NOT REG_FUNCS)
            set(REG_FUNCS "")
        endif()
        list(APPEND REG_FUNCS "${CMAKE_MATCH_1}")
        set_property(GLOBAL PROPERTY CTEST_REGISTRATION_FUNCTIONS "${REG_FUNCS}")
    else()
        get_filename_component(TEST_NAME "${TEST_FILE}" NAME)
        message(WARNING "Test file '${TEST_NAME}' has no register_*_tests() function")
    endif()
endfunction()

function(test_collect_registration_functions)
    foreach(TEST_FILE ${ARGN})
        _test_collect_registration_function("${TEST_FILE}")
    endforeach()
endfunction()

function(test_generate_registry)
    set(options "")
    ...
    if(NOT ARG_TARGET)
        message(FATAL_ERROR "test_generate_registry: TARGET is required")
    endif()
    if(NOT ARG_OUTPUT)
        message(FATAL_ERROR "test_generate_registry: OUTPUT is required")
    endif()
    ...
    list(REMOVE_DUPLICATES REG_FUNCS)
    list(SORT REG_FUNCS)

    set(REGISTRY_SOURCE "/* Auto-generated by TestDiscovery.cmake. */\n\n")

    foreach(REG_FUNC ${REG_FUNCS})
        string(APPEND REGISTRY_SOURCE "extern void ${REG_FUNC}(void);\n")
    endforeach()

    string(APPEND REGISTRY_SOURCE "\nvoid ctest_register_all_suites(void)\n{\n")
    if(REG_FUNCS)
        foreach(REG_FUNC ${REG_FUNCS})
            string(APPEND REGISTRY_SOURCE "\t${REG_FUNC}();\n")
        endforeach()
    endif()
    string(APPEND REGISTRY_SOURCE "}\n")

    file(WRITE "${ARG_OUTPUT}" "${REGISTRY_SOURCE}")
...
#       CATEGORY "unit"
#       MODULES osal hal pdi pdm pconfig aconfig
#   )
# ----------------------------------------------------------------------------
function(test_discover_category)
    ...
    foreach(MODULE ${ARG_MODULES})
        ...
        if(EXISTS "${MODULE_DIR}")
            ...
            if(MODULE STREQUAL "pconfig")
                set(LIBRARY_NAME "pconfig")
            elseif(MODULE STREQUAL "aconfig")
