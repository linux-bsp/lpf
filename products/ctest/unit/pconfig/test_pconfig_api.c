...
{
    const pconfig_platform_config_t *cfg = PCONFIG_Find("ti", "framework_alt", NULL);
    TEST_ASSERT_NOT_NULL(cfg);
...
{
    const pconfig_platform_config_t *cfg = PCONFIG_Find("missing", "framework", NULL);
    TEST_ASSERT_NULL(cfg);
}

static void test_pconfig_hw_get_mcu_by_id(void)
{
    const pconfig_platform_config_t *platform = PCONFIG_Find("ti", "framework", NULL);
...
static const test_case_t test_cases[] = {
    ...
    { .name = "test_pconfig_find_alt_config", .func = test_pconfig_find_alt_config, .setup = NULL, .teardown = NULL },
    { .name = "test_pconfig_find_missing_config", .func = test_pconfig_find_missing_config, .setup = NULL, .teardown = NULL },
