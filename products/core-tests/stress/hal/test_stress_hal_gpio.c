...
static void test_stress_gpio_high_frequency_toggle(void)
{
	...
	TEST_ASSERT_EQUAL(GPIO_STRESS_TOGGLE_COUNT * GPIO_STRESS_THREAD_COUNT, total_toggles);
 ...
 */
static int32_t gpio_concurrent_access_worker(void *user_data, uint32_t iteration)
{
	gpio_worker_ctx_t *ctx = (gpio_worker_ctx_t *)user_data;
	int32_t ret;
	hal_gpio_level_t level;
	...
	hal_gpio_level_t expected = (iteration % 2) ? HAL_GPIO_LEVEL_HIGH : HAL_GPIO_LEVEL_LOW;
	if (level == expected) {
		OSAL_atomic_inc(ctx->toggle_counter);
	}
...
static void test_stress_gpio_concurrent_access(void)
{
	...
	/* Create stress test context */
	stress_ctx = stress_context_create("GPIO_Concurrent_Access", &stress_config);
	TEST_ASSERT_NOT_NULL(stress_ctx);

	/* Run stress test */
	for (uint32_t i = 0; i < GPIO_STRESS_PIN_COUNT; i++) {
		ret = stress_run(stress_ctx, gpio_concurrent_access_worker, &worker_ctx[i]);
		TEST_ASSERT_EQUAL(OSAL_SUCCESS, ret);
	}
 ...
 */
static void gpio_interrupt_storm_callback(uint32_t pin, hal_gpio_level_t level, void *user_data)
{
...
static void register_stress_hal_gpio_tests(void)
{
